...
import { Calendar, Sun, Clock, Info, CheckCircle2, Sparkles } from 'lucide-react';
...
const MuhurtaPage = () => {
  ...
  const categories = [
    ...
    { id: 'Samskaras', label: 'Namakarana' },
    { id: 'Business', label: 'Business Launch' },
  ];

  const filteredMuhurtas = selectedCategory === 'all'
    ? muhurtasData
    : muhurtasData.filter((m) => m.category === selectedCategory);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-900 flex items-start gap-3 shadow-xs">
        ...
        <div className="space-y-1">
          <span className="font-bold text-amber-950 block">Development Sample Panchang Records</span>
          <p className="leading-relaxed">
            These records represent sample auspicious dates for demonstration. Custom location-based real-time Panchang calculation engines will be integrated in future release phases.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredMuhurtas.map((m) => (
          <Card key={m.id} className="p-6 rounded-2xl bg-white border border-stone-200 space-y-4 shadow-xs">
            ...
            <div className="bg-stone-50 rounded-xl p-4 border border-stone-100 space-y-2 text-xs text-stone-700">
              <div className="flex items-center gap-2 font-bold text-stone-800">
                ...
                <span>Window: {m.auspiciousWindow}</span>
              </div>
            ...
            <div className="pt-3 border-t border-stone-100 flex items-center justify-between">
              ...
              <Link to={`/poojas?search=${encodeURIComponent(m.category)}`}>
